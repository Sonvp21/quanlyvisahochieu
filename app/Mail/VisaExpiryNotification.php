...
class VisaExpiryNotification extends Mailable
{
    ...
    public function __construct(Student $student)
    {
        ...
        if ($student->visa && $student->visa->expiry_date) {
            ...
            $expiryDate = Carbon::parse($student->visa->expiry_date)->endOfDay();
            $now = Carbon::now();
            ...
            $this->expiryDate = Carbon::parse($student->visa->expiry_date)->format('d/m/Y');
        } else {
    ...
    public function envelope(): Envelope
    {
        $subject = $this->isExpired
            ? '⚠️ VISA ĐÃ HẾT HẠN - Cần cập nhật ngay!'
            : '⏰ NHẮC NHỞ: Visa sắp hết hạn';
